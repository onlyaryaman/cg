...
void DDA(int x1, int y1, int x2, int y2)
{
    ...
    for (int i = 0; i <= steps; i++)
    {
        ...
        {
            putpixel(round(int(X)), round(int(Y)), YELLOW);
            delay(5);
        }
        else if (X - int(X) < 0.5 && Y - int(Y) >= 0.5)
        {
            putpixel(round(int(X)), round(int(Y) + 1), YELLOW);
            delay(5);
        }
        else if (X - int(X) >= 0.5 && Y - int(Y) < 0.5)
        {
            putpixel(round(int(X) + 1), round(int(Y)), YELLOW);
            delay(5);
        }
        else
        {
            putpixel(round(int(X) + 1), round(int(Y) + 1), YELLOW);
            delay(5);
        }
...
int main()
{
    ...
    wcPt2 p1 = {70.0, 50.0}; // Start point
    wcPt2 p2 = {550.0, 280.0};
    wcPt2 p3 = {350.0, 350.0}; // End point
    wcPt2 p4 = {450.0, 400.0};
    // Draw the original line
    line(ROUND(p1.x), ROUND(p1.y), ROUND(p2.x), ROUND(p2.y));
    line(ROUND(p2.x), ROUND(p2.y), ROUND(p3.x), ROUND(p3.y));
    line(ROUND(p3.x), ROUND(p3.y), ROUND(p4.x), ROUND(p4.y));
    line(ROUND(p4.x), ROUND(p4.y), ROUND(p1.x), ROUND(p1.y));

    clipLine(winMin, winMax, p1, p2);
    clipLine(winMin, winMax, p2, p3);
    clipLine(winMin, winMax, p3, p4);
    clipLine(winMin, winMax, p4, p1);
    closegraph();
    return 0;
}